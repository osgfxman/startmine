    ...
    async function _renderGantt2() {
      ...
      try{
        ...
        for(var dayOff=0;dayOff<spDays;dayOff++){
          ...
          dw.style.cssText='display:flex;align-items:stretch;height:'+CS+'px;'+(isToday?'background:rgba(66,133,244,.07);':'')+(isFuture?'opacity:.3;':'')+(dayOff>0?'border-top:1px solid '+bdr+';':'');
          // Label cell
          var dl=document.createElement('div');
          dl.style.cssText='width:'+LW+'px;display:flex;flex-direction:column;align-items:center;justify-content:center;flex-shrink:0;line-height:1.2;border-right:1px solid '+bdr+';padding:0;margin:0;';
          var hijriDay='';try{hijriDay=new Intl.DateTimeFormat('en-u-ca-islamic-umalqura',{day:'numeric'}).format(dayDate);}catch(e){}
          var dg=document.createElement('span');dg.style.cssText='font-size:.6rem;font-weight:900;color:'+(isToday?'#4285f4':(isDk?'#ddd':'#111'))+';';dg.textContent=dayDate.getDate()+'/'+(hijriDay||'');
          var dfs=document.createElement('span');dfs.style.cssText='font-size:.4rem;font-weight:700;color:'+(dayFruitCount>0?'#e74c3c':'rgba(128,128,128,.3)')+';';dfs.textContent='\uD83C\uDF4E'+dayFruitCount+'/16';
          dl.appendChild(dg);dl.appendChild(dfs);
          dw.appendChild(dl);
          var allCells=[];
          for(var si=0;si<6;si++){
