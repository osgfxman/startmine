...
$fn = @'
    async function _renderGantt2() {
      ...
      var tH=bM*2+bH+off;
      var spY=bY+Math.floor(tH/24);tH=((tH%24)+24)%24;
      ...
      try{
        ...
        var CS=Math.max(7,Math.floor((bw-LW-6*DW)/65));
        ...
        // NO header row - time labels inside cells
        ...
        for(var dayOff=0;dayOff<spDays;dayOff++){
          ...
          // Day wrapper = horizontal: [LABEL | data-column]
          var dw=document.createElement('div');
          dw.style.cssText='display:flex;flex:1;min-height:0;'+(isToday?'background:rgba(66,133,244,.07);':'')+(isFuture?'opacity:.3;':'')+(dayOff>0?'border-top:1px solid '+bdr+';':'');
          // MERGED label spanning both rows
          var dl=document.createElement('div');
          dl.style.cssText='width:'+LW+'px;display:flex;flex-direction:column;align-items:center;justify-content:center;flex-shrink:0;line-height:1.3;border-right:1px solid '+bdr+';';
          var dg=document.createElement('span');dg.style.cssText='font-size:.55rem;font-weight:900;color:'+(isToday?'#4285f4':(isDk?'#ddd':'#111'))+';';dg.textContent=dn[dayDate.getDay()].substring(0,2)+' '+dayDate.getDate()+'/'+(dayDate.getMonth()+1);
          ...
          dw.appendChild(dl);
          // Data column: event row on top, fruit row below
          var dc=document.createElement('div');
          dc.style.cssText='display:flex;flex-direction:column;flex:1;min-width:0;';
          var r1=document.createElement('div');r1.style.cssText='display:flex;align-items:stretch;flex:1;min-height:0;';
          var r2=document.createElement('div');r2.style.cssText='display:flex;align-items:stretch;flex:1;min-height:0;border-top:1px solid '+(isDk?'rgba(255,255,255,.04)':'rgba(0,0,0,.04)')+';';
          var allCells=[];
          for(var si=0;si<6;si++){
            ...
            // Session separator
            if(si>0){
              r1.appendChild(Object.assign(document.createElement('div'),{style:{cssText:'width:'+CS+'px;flex-shrink:0;background:'+(isDk?'rgba(255,255,255,.03)':'rgba(0,0,0,.03)')+';'}}));
              r2.appendChild(Object.assign(document.createElement('div'),{style:{cssText:'width:'+CS+'px;flex-shrink:0;background:'+(isDk?'rgba(255,255,255,.03)':'rgba(0,0,0,.03)')+';'}}));
            }
            // Session wrapper (events)
            var sw1=document.createElement('div');sw1.style.cssText='display:flex;align-items:stretch;flex-shrink:0;outline:2px solid '+sessClr+';outline-offset:-2px;border-radius:2px;position:relative;'+(sessOK||isSpec?'background:rgba(39,174,96,'+(sessOK?'.06':'.08')+');':'');
            if(sess.tip)sw1.title=sess.tip;
            // Session wrapper (fruits)
            var sw2=document.createElement('div');sw2.style.cssText='display:flex;align-items:stretch;flex-shrink:0;outline:2px solid '+sessClr+';outline-offset:-2px;border-radius:0 0 2px 2px;position:relative;overflow:hidden;'+(isSpec?'background:rgba(39,174,96,.06);':'');
            LAYOUT.forEach(function(lc){
              if(lc.type==='div'){
                sw1.appendChild(Object.assign(document.createElement('div'),{style:{cssText:'width:'+DW+'px;flex-shrink:0;background:'+(sessOK?'rgba(39,174,96,.2)':bdr)+';'}}));
                sw2.appendChild(Object.assign(document.createElement('div'),{style:{cssText:'width:'+DW+'px;flex-shrink:0;background:rgba(128,128,128,.04);'}}));
                return;
              }
              if(lc.type==='gap'){
              ...
              // Time label inside cell (visible when no event)
              ...
              ec.title=tip;
              if(sEvts.length===0){var tl=document.createElement('span');tl.style.cssText='font-size:'+Math.min(CS-2,10)+'px;color:'+(isDk?'rgba(255,255,255,.15)':'rgba(0,0,0,.12)')+';font-weight:'+(m1===0?'700':'400')+';pointer-events:none;';tl.textContent=timeLabel;ec.appendChild(tl);}
          ...
          dc.appendChild(r1);dc.appendChild(r2);dw.appendChild(dc);daysCt.appendChild(dw);
        }
