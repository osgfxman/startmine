...
try {
    ...
    if (match && match[1]) {
        let rawBase64 = match[1].replace(/\s/g, '');
        let buffer = Buffer.from(rawBase64, 'base64');

        // Test a full decoding brute force from 1 to 255
        let found = false;
        for (let offset = 0; offset < 256; offset++) {
            let decoded = '';
            for (let i = 0; i < buffer.length; i++) {
                decoded += String.fromCharCode((buffer[i] + offset) % 256);
            }

            if (decoded.startsWith('{') && decoded.includes('"data"')) {
                console.log(`BINGO! Offset found: ${offset}`);
                console.log(decoded.substring(0, 50));
                const j = JSON.parse(decoded);
                console.log("Valid JSON with items: ", j.data.objects.length);
                found = true;
                break;
            }

            // Also test subtractive
            let decodedSub = '';
            for (let i = 0; i < buffer.length; i++) {
                decodedSub += String.fromCharCode((buffer[i] - offset + 256) % 256);
            }
            if (decodedSub.startsWith('{') && decodedSub.includes('"data"')) {
                console.log(`BINGO SUBTRACTIVE! Offset found: ${offset}`);
                console.log(decodedSub.substring(0, 50));
                const j = JSON.parse(decodedSub);
                console.log("Valid JSON with items: ", j.data.objects.length);
                found = true;
                break;
            }
