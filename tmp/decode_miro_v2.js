const rawMeta = "<--(miro-data-v1)(/miro-data-v1)-->";

const match = rawMeta.match(/<--\(miro-data-v1\)([\s\S]*?)\(\/miro-data-v1\)-->/);
if (!match) { console.log('No miro data found'); process.exit(1); }

let b64 = match[1].replace(/\s/g, '').replace(/-/g, '+').replace(/_/g, '/');
while (b64.length % 4) b64 += '=';
const raw = Buffer.from(b64, 'base64').toString('binary');
const firstByte = raw.charCodeAt(0);
const key = (123 - firstByte + 256) % 256;

// Decode to bytes first, then handle as UTF-8
const bytes = [];
for (let i = 0; i < raw.length; i++) {
    bytes.push((raw.charCodeAt(i) + key) % 256);
}
const decoded = Buffer.from(bytes).toString('utf8');

try {
    const miroJson = JSON.parse(decoded);

    // Show structure overview with positions
    if (miroJson.data && miroJson.data.objects) {
        console.log('Total objects:', miroJson.data.objects.length);
        miroJson.data.objects.forEach((obj, i) => {
            const wd = obj.widgetData;
            if (wd && wd.json) {
                const jd = wd.json;
                console.log(`\n--- Object ${i} ---`);
                console.log('Type:', wd.type);
                if (jd._position) console.log('Position:', JSON.stringify(jd._position));
                if (jd.scale) console.log('Scale:', JSON.stringify(jd.scale));
                if (jd.size) console.log('Size:', JSON.stringify(jd.size));
                if (jd.text) console.log('Text:', jd.text.substring(0, 50));
                if (jd.url) console.log('URL:', jd.url);
                if (jd.resourceUri) console.log('ResourceUri:', jd.resourceUri);
            }
        });
    }
} catch (e) {
    console.error('JSON parse error:', e.message);
    // Try to show partial
    const decoded2 = Buffer.from(bytes).toString('utf8');
    console.log('Decoded (first 2000 chars):', decoded2.substring(0, 2000));
}
