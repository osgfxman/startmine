...
$fn = @'
    async function _renderGantt2() {
      ...
      try{
        ...
        var bw=body.clientWidth-4,bh=body.clientHeight-4;
        // Card layout: each day = card with 6 session rows
        // Card width = 10 cells + 2 gaps + 1 divider per session row
        // Cell size based on height: 6 session rows + header = 7 rows
        var CS=Math.max(8,Math.floor(bh/8));
        var DW=2;
        var cardW=10*CS+2*DW; // session row = 8 data + 2 gap cells = 10*CS + 1 divider
        var ct=document.createElement('div');
        ct.style.cssText='display:flex;flex-direction:row-reverse;height:100%;box-sizing:border-box;font-family:var(--font);overflow-x:auto;overflow-y:hidden;padding:1px 2px;gap:3px;align-items:stretch;';
        for(var dayOff=0;dayOff<spDays;dayOff++){
          ...
          card.style.cssText='display:flex;flex-direction:column;flex-shrink:0;width:'+cardW+'px;border:1px solid '+(isToday?'#4285f4':bdr)+';border-radius:4px;'+(isToday?'background:rgba(66,133,244,.07);box-shadow:0 0 4px rgba(66,133,244,.3);':'')+(isFuture?'opacity:.3;':'');
          // Card header
          var hijriDay='';try{hijriDay=new Intl.DateTimeFormat('en-u-ca-islamic-umalqura',{day:'numeric'}).format(dayDate);}catch(e){}
          var dn=['Su','Mo','Tu','We','Th','Fr','Sa'];
          var hdr=document.createElement('div');
          hdr.style.cssText='display:flex;align-items:center;justify-content:center;gap:3px;height:'+Math.max(14,CS*0.6)+'px;border-bottom:1px solid '+bdr+';flex-shrink:0;';
          var hd1=document.createElement('span');hd1.style.cssText='font-size:.55rem;font-weight:900;color:'+(isToday?'#4285f4':(isDk?'#ddd':'#111'))+';';hd1.textContent=dn[dayDate.getDay()]+' '+dayDate.getDate()+'/'+(hijriDay||'');
          var hd2=document.createElement('span');hd2.style.cssText='font-size:.35rem;font-weight:700;color:'+(dayFruitCount>0?'#e74c3c':'rgba(128,128,128,.3)')+';';hd2.textContent='\uD83C\uDF4E'+dayFruitCount+'/16';
          ...
          for(var si=0;si<6;si++){
            ...
            sr.style.cssText='display:flex;align-items:stretch;height:'+CS+'px;flex-shrink:0;outline:1px solid '+sessClr+';outline-offset:-1px;position:relative;'+(sessOK||isSpec?'background:rgba(39,174,96,'+(sessOK?'.06':'.08')+');':'')+(si>0?'border-top:1px solid '+bdr+';':'');
            ...
            LAYOUT.forEach(function(lc){
              ...
              if(lc.type==='gap'){
                ...
                if(fz){gp.textContent='\uD83C\uDF4C';}else{gp.textContent=sessDead?'\u2620\uFE0F':'\uD83E\uDD29';}
